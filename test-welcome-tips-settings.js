...
try {
  ...
  // 2. Save showWelcomeTips: true and check persistence
  Promise.resolve(save({}, { ...defaults, showWelcomeTips: true }))
    .then((result) => {
      ...
      console.log('Welcome tips settings tests passed successfully!');
    })
    .catch((error) => {
      console.error(error);
      process.exitCode = 1;
