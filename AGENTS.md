...
- **Two electron-builder configs exist: `electron-builder.yml` and the `build` key in `package.json`. `electron-builder.yml` wins.** It references `build/` assets (`build/icon.icns`, `build/installer.nsh`) that do NOT exist in the repo; only `public/icon.png` does. Prefer editing `electron-builder.yml` and be aware a mac/win build may need those missing assets.
- `electron-builder.yml` `publish` targets upstream `groq/groq-desktop-beta`, but the git remote is the `giseldo/groq-desktop-beta` fork.
