...
function Test-ScriptExistence {
    ...
    $scriptsBase = if (Test-Path "$PSScriptRoot/../electron/scripts") { "$PSScriptRoot/../electron/scripts" } else { "./electron/scripts" }
    $scriptPath = Join-Path -Path $scriptsBase -ChildPath $ScriptName
